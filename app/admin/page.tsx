...
export default function AdminDashboard() {
  ...
  // --- ANALYTICS LOGIC (Realtime dari data Orders) ---
  const analyticsData = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      // Hitung total di tanggal tersebut
  ...
  const dashboardStats = useMemo(() => {
    ...
    if (filterType === "daily" && dashboardDate) {
      filtered = completed.filter((o) => o.createdAt.startsWith(dashboardDate));
    } else if (filterType === "monthly" && dashboardMonth) {
      filtered = completed.filter((o) =>
        o.createdAt.startsWith(dashboardMonth),
      );
    }
    ...
    // Aggregate Items (Hitung item yang terjual)
          ...
          <div className="flex flex-col md:flex-row items-center gap-4">
            <div className="bg-white p-1.5 rounded-full shadow-sm border border-stone-200 flex">
              <button
                ...
                className={`px-6 py-2.5 rounded-full text-sm font-bold transition-all ${activeTab === "DASHBOARD" ? "bg-[#2d241e] text-white shadow-md" : "text-stone-500 hover:bg-stone-50"}`}
              >
                📊 Home
              </button>
              <button
                onClick={() => setActiveTab("MENU")}
                className={`px-6 py-2.5 rounded-full text-sm font-bold transition-all ${activeTab === "MENU" ? "bg-[#2d241e] text-white shadow-md" : "text-stone-500 hover:bg-stone-50"}`}
              >
                📦 Menu
              </button>
              <button
                onClick={() => setActiveTab("ORDERS")}
                className={`px-6 py-2.5 rounded-full text-sm font-bold transition-all flex items-center gap-2 ${activeTab === "ORDERS" ? "bg-[#2d241e] text-white shadow-md" : "text-stone-500 hover:bg-stone-50"}`}
              >
                🔔 Pesanan
            ...
            <div className="bg-white p-4 rounded-3xl shadow-sm border border-stone-200 flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-2 bg-stone-100 p-1 rounded-full">
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${filterType === "daily" ? "bg-white shadow text-stone-800" : "text-stone-500"}`}
                >
                  Harian
                </button>
                <button
                  onClick={() => setFilterType("monthly")}
                  className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${filterType === "monthly" ? "bg-white shadow text-stone-800" : "text-stone-500"}`}
                >
                  Bulanan
                </button>
              </div>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-6">
              <div className="flex gap-2">
                <button
                  onClick={() =>
                    setBulkModal({ show: true, targetStatus: true })
                  }
                ...
                <button
                  onClick={() =>
                    setBulkModal({ show: true, targetStatus: false })
                  }
              ...
              <div className="flex-1 overflow-x-auto pb-1 flex gap-2 scrollbar-hide">
                {categories.map((cat) => (
                  <button
                    ...
                    className={`px-5 py-2 rounded-full text-xs font-bold border shrink-0 ${selectedCategory === cat ? "bg-[#2d241e] text-white border-[#2d241e]" : "bg-white text-stone-500 border-stone-200"}`}
                  >
                    {cat}
                  </button>
                ))}
                    ...
                    : filteredMenu.map((item) => (
                        <tr key={item._id} className="hover:bg-stone-50/50">
                          ...
                          <td className="px-6 py-4 font-bold text-amber-800">
                            Rp{" "}
                            {item.variants?.[0]?.price.toLocaleString("id-ID")}
                          </td>
                          <td className="px-6 py-4">
                            <button
                              onClick={() => toggleStatus(item)}
                              className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${item.isAvailable ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                            >
                  ...
                  filteredMenu.map((item) => (
                    ...
                    >
                      ...
                      <div className="flex gap-2 mt-4 pt-4 border-t border-stone-100">
                        <button
                          onClick={() => toggleStatus(item)}
                          className={`flex-1 text-[10px] font-bold uppercase rounded-xl py-2 ${item.isAvailable ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                        >
            ...
            <div className="md:hidden space-y-4">
              {isLoadingOrders ? (
                ...
                filteredOrders.map((order) => (
                  <div
                    key={order._id}
                    className={`p-6 rounded-3xl shadow-sm border flex flex-col gap-4 ${order.status === "completed" ? "bg-stone-50 border-stone-100 opacity-70" : "bg-white border-stone-200"}`}
                  >
                  ...
                  <tbody className="divide-y divide-stone-50">
                    ...
                    {isLoadingOrders ? (
                      ...
                      filteredOrders.map((order) => (
                        <tr
                          key={order._id}
                          className={`hover:bg-stone-50/50 transition-colors ${order.status === "completed" ? "opacity-60 bg-stone-50" : ""}`}
                        >
              ...
              <div className="flex-1 p-8 overflow-y-auto custom-scrollbar">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-stone-50 p-5 rounded-3xl border border-stone-100">
                    ...
                    <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
                      {editingData.variants.map((v, i) => (
                        <div key={i} className="flex gap-2 items-center">
                          ...
                          <button
                            onClick={() =>
                              modifyVariantCount("remove", i, true)
                            }
                    ...
                    <button
                      onClick={() => modifyVariantCount("add", undefined, true)}
                      className="w-full mt-3 py-2 border-2 border-dashed border-stone-200 text-stone-400 rounded-xl text-xs font-bold hover:border-amber-400 hover:text-amber-600 transition-colors"
                    >
                      + Tambah Varian
                    </button>
