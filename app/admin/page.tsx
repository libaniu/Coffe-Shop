...
export default function AdminDashboard() {
  ...
  const handleImageUpload = (
    ...
    if (file) {
      if (file.size > 1024 * 1024)
        return alert("File Terlalu Besar, Maksimal 1MB");
  ...
  const handleSaveMenu = async () => {
    if (!newMenu.name || !newMenu.price || !newMenu.img)
      return alert("Lengkapi Data Menu!!!");
    const res = await fetch("/api/menu", {
      method: "POST",
  ...
  const toggleStatus = async (item: IMenu) => {
    ...
    if (res.ok) {
      setMenuList((prev) =>
        prev.map((menu) =>
          menu._id === item._id ? { ...menu, isAvailable: newStatus } : menu
        )
      );
      fetchLogs();
    }
  ...
  const executeBulkStatus = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setMenuList((prev) =>
          prev.map((item) => ({ ...item, isAvailable: status }))
        );
      ...
      console.error("Gagal update masal.");
    }
  };
    ...
    <div className="p-3 md:p-10 bg-stone-50 min-h-screen text-[#2d241e]">
      <div className="max-w-5xl mx-auto pb-20">
        ...
        <div className="flex justify-between items-center mb-8 px-2">
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2 rounded-full bg-red-50 text-red-600 font-bold text-xs hover:bg-red-500 hover:text-white transition-all shadow-sm"
          >
            Logout <span>➔</span>
          </button>
        </div>
        ...
        <div className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-sm border border-stone-200 mb-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
            <div className="space-y-4">
              ...
              <input
                type="file"
                accept="image/*"
                className="text-sm text-stone-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-[#2d241e] file:text-white"
          ...
          <button
            ...
            className="w-full mt-6 bg-[#2d241e] text-white py-4 rounded-2xl font-bold hover:bg-amber-900 transition-all shadow-lg active:scale-95"
          >
            Simpan Menu
          </button>
        </div>
        ...
        <div className="mb-20">
          ...
          <div className="md:hidden space-y-4 px-2">
            {isLoading ? (
                ...
                >
                  ...
                  <div className="flex gap-2 border-t pt-4">
                    <button
                      ...
                      className="flex-1 bg-amber-50 text-amber-700 py-3 rounded-xl text-[10px] font-bold uppercase tracking-wider"
                    >
                      Edit
                    </button>
                    <button
                      ...
                      className="flex-1 bg-red-50 text-red-500 py-3 rounded-xl text-[10px] font-bold uppercase tracking-wider"
                    >
                      Hapus
                    </button>
                  </div>
              ...
              <p className="text-center py-10 italic text-stone-400">
                Menu tidak ditemukan.
              </p>
            )}
          </div>

          {/* 2. VIEW DESKTOP (TABLE) */}
          <div className="hidden md:block bg-white rounded-4xl shadow-sm border border-stone-200 overflow-hidden">
        ...
        <div className="mt-20 px-2">
          <div className="flex items-center gap-4 mb-8">
            <div className="w-10 h-10 rounded-full bg-amber-50 flex items-center justify-center text-xl shadow-inner">
              📜
            </div>
            <div>
              ...
              <p className="text-[10px] text-stone-400 uppercase tracking-[0.2em] font-black italic">
                System Logs
              </p>
            </div>
          </div>
          <div className="space-y-3">
            {isLoading ? (
              ...
              logs.map((log) => (
                <div
                  key={log._id}
                  className="flex items-center justify-between p-5 bg-white border border-stone-200 rounded-4xl hover:shadow-md transition-all"
                >
      ...
      {isEditModalOpen && editingData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[3rem] p-8 md:p-10 max-w-2xl w-full shadow-2xl">
            <h2 className="text-2xl font-serif font-bold mb-8 text-[#2d241e]">
              Edit Menu
            </h2>
            ...
            <div className="flex gap-4 mt-10">
              <button
                ...
                className="flex-1 py-4 bg-[#2d241e] text-white rounded-2xl font-bold"
              >
                Simpan
              </button>
              <button
                onClick={() => setIsEditModalOpen(false)}
                className="flex-1 py-4 bg-stone-100 text-stone-500 rounded-2xl font-bold"
              >
                Batal
              </button>
            </div>
      ...
      {deleteModal.show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-stone-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[3rem] p-10 max-w-sm w-full text-center shadow-2xl">
            <h3 className="text-xl font-bold text-[#2d241e] mb-2">
              Hapus Menu
            </h3>
            <p className="text-stone-500 mb-8 text-sm italic">
              Hapus <span className="font-bold">"{deleteModal.name}"</span>?
            </p>
      ...
      {bulkModal.show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-stone-900/60 backdrop-blur-md">
          <div className="bg-white rounded-[3rem] p-10 max-w-sm w-full text-center shadow-2xl">
            <h3 className="text-xl font-bold text-[#2d241e] mb-4">
              Konfirmasi Status
            </h3>
            <p className="text-stone-500 mb-8 text-sm italic">
              Setel SEMUA MENU menjadi{" "}
              <span
                className={
                  bulkModal.targetStatus ? "text-green-600" : "text-red-600"
                }
              >
        ...
        <div className="fixed inset-0 z-60 flex items-center justify-center p-6 bg-stone-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[3rem] p-10 max-w-sm w-full text-center shadow-2xl">
            ...
            <button
              ...
              className="w-full py-4 bg-[#2d241e] text-white rounded-2xl font-bold"
            >
              Selesai
            </button>
          </div>
